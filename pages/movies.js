...
const style = {
    ...
    mainCont: `flex justify-between gap-5 pt-[5rem] px-10 text-white text-2xl`,
    ...
    listCont: `flex flex-col gap-5 w-screen text-white h-full bg-[#141414]  text-4xl`,
...
const Movies = () => {
    ...
    const {movie, mobileMenue} = useContext(NetflixContext);
        ...
        switch (option) {
            case 'action':
                return (
                    ...
                    { 
                    movie
                    .filter(number => number.genres[0] === 28)
                    .map((number, index) => (
            ...
            case 'romance':
                return  (
                    ...
                    { 
                    movie
                    .filter(number => number.genres[1] === 10749)
                    .map((number, index) => (
                    <img src={number.posterURLs.original} width={200} alt="" key={index} />
                    )) 
                    }
